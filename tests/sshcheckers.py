...
def ssh_co(host, user, passwd, cmd, text="", port=22):
...
def put_files(host, user, passwd, local_path, remote_path, port=22):
...
# def get_files(host, user, passwd, remote_path, local_path, port=22):
...
def ssh_co_negative(host, user, passwd, cmd, text, port=22):
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(hostname=host, username=user, password=passwd, port=port)
    stdin, stdout, stderr = client.exec_command(cmd)
